  ...
  return (
    <div className='relative'>
      {error && <span className={clsx(
        (styleType === 'answer') && 'absolute left-1/2 -translate-x-1/2 -top-6.5 px-2 border border-neutral-500 rounded-sm bg-neutral-600',
        (styleType === 'comment') && 'absolute left-1/2 translate-x-5 -top-8 px-2',
        ' text-red-300'
      )}>{error.message}</span>}
  ...
  return (
    <div className='flex-1 text-left'>
      {TEXTAREA_BUTTONS.map(({ title, tagName, attributes }) => (
        <div
          className='inline-block px-3 py-1 font-bold border-r cursor-pointer hover:bg-amber-50/10'
          key={title}
...
function Preview({ isPreviewHidden, togglePreview }) {
  return (
    <div className='flex-1 text-center'>
      <img
        className='inline px-8 py-0.5 h-8 font-bold tracking-widest border-x cursor-pointer hover:bg-amber-50/10'
